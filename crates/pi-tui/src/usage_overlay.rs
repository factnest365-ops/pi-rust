...
impl UsageOverlay {
    ...
    pub fn render_summary(tokens: usize, max_limit: usize) -> Vec<Line<'static>> {
        ...
        vec![
            Line::from(vec![
                Span::styled("Context Window Capacity: ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            ]),
            Self::render_progress_bar(pct, 25),
            Line::from(vec![
                Span::styled(format!("Tokens Used: {} / {} (Remaining: {})", tokens, max_limit, max_limit.saturating_sub(tokens)), Style::default().fg(Color::DarkGray)),
            ]),
        ]
    }
}
