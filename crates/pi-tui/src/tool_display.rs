...
impl ToolDisplay {
    ...
    pub fn render_executing_styled(tool_name: &str, tool_call_id: &str, theme: &ThemePalette) -> Vec<Line<'static>> {
        vec![
            Line::from(vec![
                Span::styled("⚙ ", Style::default().fg(theme.yellow)),
                Span::styled("Executing Tool: ", theme.tool_label()),
                Span::styled(tool_name.to_string(), Style::default().fg(theme.text).add_modifier(Modifier::BOLD)),
                Span::styled(format!(" (call_id: {})", tool_call_id), Style::default().fg(theme.muted)),
            ]),
        ]
    }
    ...
    pub fn render_completed_styled(tool_name: &str, is_error: bool, theme: &ThemePalette) -> Vec<Line<'static>> {
